...
1. **Prepare the Environment**: Ensure you are inside the directory /home/kgcorp/docker-app (or wherever your Dockerfile and src folder are located).
2. **Build the Image**: Create the Docker image from your Dockerfile.
   bash command - docker build -t kgcorp/python-app .
3. **Run the Container**: Start the container in detached mode (-d).
...
4. **Verify the Status**: Check if the container is up and running. Look for the "Up" status and the correct port mapping.
   bash command - docker ps -a
5. **Test the Application**: Use curl to confirm the web server is responding with your "Welcome" message.
